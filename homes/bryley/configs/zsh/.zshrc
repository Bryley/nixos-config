...
# Some useful options
stty stop undef                     # Disable Ctrl-S from freezing screen zle_highlight=('paste:none')        # Stop pasted from being highlighted
...
if [ $(fast-theme --show | grep -o base16 | wc -c) -eq 0 ]; then
    fast-theme base16   # Set fast-syntax-highlighting theme automatically
fi
# End of plugins

# Sourcing extra files
if [ -e /etc/profile.d/command-not-found.sh ]; then
